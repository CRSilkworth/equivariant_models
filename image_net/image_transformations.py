...
def add_horizontal_flip_to_batch(images, labels, to_run=None):
    ...
    stacked = tf.concat([images, flipped], axis=0)
    
    return stacked, labels


def add_five_crops_to_batch(images, labels, image_size, crop_image_size, to_run=None):
    # Four corner crops
    ...
    crop_h = (image_size[0] - crop_image_size[0])/2
    crop_w = (image_size[1] - crop_image_size[1])/2
    crop_5 = images[:, crop_h: crop_h + crop_image_size[0], crop_w: crop_w + crop_image_size[1], :]
